...
class FetchSingleComplane extends StatefulWidget {
  const FetchSingleComplane({super.key});

  @override
  State<FetchSingleComplane> createState() => _FetchSingleComplaneState();
}

class _FetchSingleComplaneState extends State<FetchSingleComplane> {
  ...
  void initState() {
    super.initState();
    controller.fetchSingleQueryCom(Get.arguments['number'],loader,);
    status = Get.arguments['status'];
    complainNumber = Get.arguments['number'];
    id = Get.arguments['booking'];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Obx(() {
        ...
        }else if(controller.fetchSingleComList.length == 0){
            ...
            children: [
              Expanded(
                  ...
                  child: ListView.builder(
                    reverse: true,
                    shrinkWrap: true,
                    itemCount: controller.fetchSingleComList.length,
                    itemBuilder: (BuildContext context, int index) {
                      var reversedList = controller.fetchSingleComList;
                      return Padding(
                ...
                child: Obx(() {
                  if (controller.closeQueryLoader.value) {
                    ...
                    return custom_button(voidCallback: () {
                      controller.closeTicket(complainNumber,id);
                    },
                      text: "Close Ticket",
                    );
                  }
